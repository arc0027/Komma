...
@section('header')
    ...
    @section('contenido')
        ...
        <div class="flex justify-center">
            <form id="formulario" method="POST" action="/contacto" class="w-3/12 h-auto m-6 pt-8 pb-8 pr-16 pl-16 text-3xl">
                @csrf
                ...
                <input type="email" id="email" name="email"
                    ...
                    placeholder="Email" value={{ old('email') }} @if (Auth::user() != null) {{ auth()->user()->email }} @endif>
                @error('email')
                ...
                <input type="email" id="email_confirmation" name="email_confirmation"
                    class="flex w-full py-3 px-4 mb-6 text-center border-2 rounded leading-tight focus:outline-none focus:border-yellow-600"
                    placeholder="Confirmacion email" value={{ old('email') }} @if (Auth::user() != null) {{ auth()->user()->email }} @endif>
                ...
                <input type="text" id="name" name="name"
                    class="flex w-full py-3 px-4 mb-6 text-center border-2 rounded leading-tight focus:outline-none focus:border-yellow-600"
                    placeholder="Nombre" value={{ old('name') }} @if (Auth::user() != null) {{ auth()->user()->name }} @endif>
                @error('name')
                    <p id="validacion" class="text-white">
                        {{ $message }}
                    </p>
